...
public class KickoutSessionFilter extends AccessControlFilter {
	...
	protected boolean isAccessAllowed(ServletRequest request,
		...
		Serializable ser = session.getId();
		String sessionId=ser.toString();
		/**
		 * 判断是否已经踢出
		...
		Boolean marker=cache.get1(sessionId);
		if (null != marker && marker ) {
			...
			session.stop();//清空当前session,并且删除redis里面的session
		...
		LinkedHashMap<Long, Serializable> infoMap = cache.get(String.valueOf(userId), LinkedHashMap.class);
		//如果不存在，创建一个新的
		...
		if(infoMap.containsKey(userId) && infoMap.containsValue(sessionId)){
			...
			cache.setex(sessionId, infoMap, 3600);
			return Boolean.TRUE;
		}
		...
		if(infoMap.containsKey(userId) && !infoMap.containsValue(sessionId)){
			...
			if(null != oldSession){
				cache.set1(oldSession.getId().toString(),"true");
				//标记session已经踢出
		...
		if(!infoMap.containsKey(userId) && !infoMap.containsValue(sessionId)){
			...
			//存储到缓存1个小时（这个时间最好和session的有效期一致或者大于session的有效期）
			cache.setex(String.valueOf(userId), infoMap, 3600);
		}
		return Boolean.TRUE;
	}
